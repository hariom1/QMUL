...
class SAModule():
    def __init__(
        ...
        print_msg_box(
            msg=f"Starting Situational Awareness module...\nTopology: {topology}", indent=2, title="Situational Awareness module"
        )
    ...
    async def reconnect_to_federation(self):
        ...
        if len(self.np.get_nodes_known()) > 0:
            ...
            await self.nm.start_late_connection_process(connected=False, msg_type="discover_nodes", addrs_known=self.np.get_nodes_known())
        else:
    ...
    async def upgrade_connection_robustness(self, possible_neighbors):
        ...
        if len(possible_neighbors) > 0:
            ...
            await self.nm.start_late_connection_process(connected=True, msg_type="discover_nodes", addrs_known=possible_neighbors)
        else:
